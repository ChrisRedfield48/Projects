(() => {
  ...
  let lastFwd=0, lastBack=0, vignette=null, bg=null;
  ...
  function bakeBackground(){
    ...
    for (const nb of nebula){
      const cxp = nb.x*W, cyp = nb.y*H, rad = nb.r*Math.max(W,H);
      const g = b.createRadialGradient(cxp, cyp, 0, cxp, cyp, rad);
  ...
  function buildNebula(){
    nebula = [
      { x:0.22, y:0.30, r:0.55, col:[160,107,255], a:0.16 },
      { x:0.80, y:0.66, r:0.62, col:[56,197,255], a:0.13 },
      { x:0.55, y:0.85, r:0.50, col:[79,255,196], a:0.08 },
      { x:0.92, y:0.12, r:0.42, col:[108,123,255], a:0.10 }
    ];
  }
  function buildNet(){
  ...
  function draw(){
    if (bg) ctx.drawImage(bg, 0, 0, W, H); else { ctx.fillStyle = C.void; ctx.fillRect(0,0,W,H); }
  ...
  addEventListener('click', e => { if (e.target.closest('a, nav, .panel, .card, button')) return; burst(e.clientX, e.clientY); });
...
(() => {
  ...
  const PROJECTS = [
    ...
    { name:'Birthday Search', tag:['JS','Data'], info:'Поиск дня рождения жильца по имени. Вложенные объекты, перебор данных.', open:'https://chrisredfield48.github.io/voda/' },
    ...
    { name:'Quiz App', tag:['JS','DOM'], info:'Викторина: счёт очков, навигация по вопросам.', open:null },
    { name:'Todo List', tag:['JS','DOM'], info:'Список задач: добавление, удаление, работа с DOM.', open:null },
    { name:'Weather App', tag:['JS','API'], info:'Погода через fetch-запросы к API.', open:null }
  ];

  const grid = document.getElementById('projects-grid');
  const live = PROJECTS.filter(p => p.open).length;
  const wip = PROJECTS.length - live;
  const lc = document.getElementById('liveCount'); if (lc) lc.textContent = live;
  const wc = document.getElementById('wipCount'); if (wc) wc.textContent = wip;

  let liveIdx = 0;
  grid.innerHTML = PROJECTS.map(p => {
    const isLive = !!p.open;
    const num = isLive ? String(++liveIdx).padStart(2,'0') : '··';
    const tags = p.tag.map(t => `<span class="tag">${esc(t)}</span>`).join('');
    const inner = `
      <div class="card-top"><span class="idx">${num}</span><span class="stat ${isLive?'':'wip'}"><span class="d"></span>${isLive?'задеплоено':'в разработке'}</span></div>
      <h3>${esc(p.name)}</h3>
      <p>${esc(p.info)}</p>
      <div class="tags">${tags}</div>
      ${isLive ? '<span class="open">открыть <span aria-hidden="true">→</span></span>' : '<span class="soon">скоро</span>'}`;
    return isLive
      ? `<a class="panel card rv" href="${esc(p.open)}" target="_blank" rel="noopener noreferrer">${inner}</a>`
      : `<div class="panel card wip rv">${inner}</div>`;
  }).join('');
